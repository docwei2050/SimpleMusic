...
{
    SimpleAudio *audio = static_cast<SimpleAudio *>(data);
    while(audio->playStatus != NULL && !audio->playStatus->exit)
    {
       PcmBean *pcmBean = NULL;
        ...
        if(pcmBean->buffsize <= audio->defaultPcmSize)//不用分包
        {
            ...
            if(audio->showPcm)
            {
                audio->callJava->onCallPcmInfo(CHILD_THREAD,pcmBean->buffer, pcmBean->buffsize);
            }
        } else{
            ...
            for(int i = 0; i < pack_num; i++)
            {
                ...
                if(audio->showPcm)
                {
                    audio->callJava->onCallPcmInfo(CHILD_THREAD,bf, audio->defaultPcmSize);
                }
                free(bf);
                bf = NULL;
            }
            ...
            if(pack_sub > 0)
            {
                ...
                if(audio->showPcm)
                {
                    audio->callJava->onCallPcmInfo(CHILD_THREAD,bf, pack_sub);
                }
            }
        }
        delete(pcmBean);
...
void SimpleAudio::play() {
    ...
    bufferQueue = new PcmBufferQueue(playStatus);
