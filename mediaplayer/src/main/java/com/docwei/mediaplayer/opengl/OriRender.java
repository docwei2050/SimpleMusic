...
import com.docwei.mediaplayer.R;
 ...
 */
public class OriRender implements GLSurfaceView.Renderer, SurfaceTexture.OnFrameAvailableListener {
    public static final int RENER_YUV=1;
    public static final int RENDER_MEDIACODEC=2;
    private Context context;
    private final float[] vertexData = {
            -1f, -1f, 1f, -1f, -1f, 1f, 1f, 1f
    };
    private final float[] textureData = {0f, 1f,
            1f, 1f, 0f, 0f, 1f, 0f
    };
    private FloatBuffer vertexBuffer;
    private FloatBuffer textureBuffer;
    private int renderType=RENER_YUV;
    ...
    //meidacodec
    private int program_mediaocodec;
    ...
    public OriRender(Context context) {
        this.context = context;
        vertexBuffer = ByteBuffer.allocateDirect(vertexData.length * 4)
                .order(ByteOrder.nativeOrder()).asFloatBuffer().put(vertexData);
        vertexBuffer.position(0);
        textureBuffer = ByteBuffer.allocateDirect(textureData.length * 4).order(ByteOrder.nativeOrder())
                .asFloatBuffer()
                .put(textureData);
    ...
    public void onDrawFrame(GL10 gl) {
        ...
        if (renderType == RENDER_MEDIACODEC) {
            renderMediacodec();
        } else if (renderType == RENER_YUV) {
            renderYUV();
        }
    ...
    private void inintRenderMediacodec() {
        String vertexSource = ShaderUtil.readRawTxt(context, R.raw.vertex_shader);
        String fragmentSource = ShaderUtil.readRawTxt(context, R.raw.fragment_mediacodec);
        program_mediaocodec = ShaderUtil.createProgram(vertexSource, fragmentSource);

        avPosition_mediacodec = GLES20.glGetAttribLocation( program_mediaocodec , "av_Position");
        afPosition_meidacodec= GLES20.glGetAttribLocation( program_mediaocodec , "af_Position");
        samplerOES_mediacodec = GLES20.glGetUniformLocation( program_mediaocodec , "sTexture");
    ...
    private void renderMediacodec() {
        ...
        GLES20.glUseProgram(program_mediaocodec);
